# Fix specific quality-gate violations: Phase 3 — audit and notification modules

You are an autonomous coding agent working INSIDE an already-cloned git repository at `/tmp/gestalt/fix/babd3932-368b-46a5-a4dc-6dccaafd84ba/3/1`. Do not clone anything; work only in this directory.
...
- The service's client parameter must match the existing repository signature: optional PoolClient as the last parameter, defaulting to the shared pool when omitted — mirror INotificationRepository.create(input, client?). (see `src/modules/notification/notification.repository.interface.ts`)
- The forwarded client must be the same PoolClient type and reuse semantics defined by the transaction-boundary decision (service owns the unit of work, data-access opens it; repositories take optional last-param client). (see `AGENTS.md`)
- The service interface change must stay consistent with the public entry point exports so the updated INotificationService signature is re-exported unchanged. (see `src/modules/notification/index.ts`)
### Entity invariants — enforce these
- Reuse or extend `Notification`: A Notification created through the service is always persisted with a status (defaulting to PENDING when omitted) and a generated id/createdAt; forwarding a client does not alter these creation semantics.
### Interface contract — expose these operations (their shape is yours)
- NotificationService.create(input, client?) — No auth rule at this layer — the service is invoked by an already-authorized caller; RBAC remains a route-level concern (out of scope for this phase).; Throws ValidationError on invalid input before any repository call; otherwise propagates repository errors unchanged. No new error types introduced.
### Integration points — connect to these
- src/modules/leave (Phase 6 orchestrator) — The leave approve/reject transaction will call NotificationService.create with the transaction client so the notification insert joins the same atomic unit as the status change, balance update, and audit insert.
...
Line: 10
Offending code: `async create(input: CreateNotificationInput): Promise<Notification> {`
Rule violated: notification-transaction-boundary
Action (do this now): Edit `src/modules/notification/notification.service.ts` at line 10 in place to fix the `notification-transaction-boundary` violation.
What the quality gate found — apply this: [notification-transaction-boundary] The intent and success criterion #6 require notifications to be synchronous direct inserts that join the caller's transaction boundary ("the service inserts within the caller's transaction boundary"). The repository accepts an optional trailing PoolClient, but NotificationService.create does not accept or forward a client — it always inserts via the shared pool outside any transaction. The service therefore cannot participate in a caller's transaction, contradicting the stated requirement.
