...
You are an autonomous coding agent working INSIDE an already-cloned git repository at `/tmp/gestalt/fix/babd3932-368b-46a5-a4dc-6dccaafd84ba/3/2`. Do not clone anything; work only in this directory.
...
- The mapped AuditLog must satisfy the AuditLog interface declared in audit.model.ts (beforeState/afterState: unknown | null), and the parse must be the inverse of the JSON.stringify performed in create() so round-trips are lossless. (see `src/modules/audit/audit.model.ts`)
- The parse behavior must be the exact inverse of the existing JSON.stringify in create() within audit.repository.ts, so values written by create() are returned structurally equal by findById(). (see `src/modules/audit/audit.repository.ts`)
### Entity invariants — enforce these
- Reuse or extend `AuditLog`: beforeState and afterState remain `unknown | null`; after mapping, a non-null column yields the parsed value and a null column yields null.
...
Line: 24
Offending code: `beforeState: row.before_state,`
Rule violated: review/correctness
Action (do this now): Edit `src/modules/audit/audit.repository.ts` at line 24 in place to fix the `review/correctness` violation.
What the quality gate found — apply this: [review/correctness] The create path JSON-stringifies beforeState/afterState before insert (`input.beforeState === null ? null : JSON.stringify(input.beforeState)`), but mapRow returns the raw column value without JSON.parse. A getById round-trip therefore returns beforeState/afterState as a JSON string (e.g. '{"status":"SUBMITTED"}') rather than the original object, breaking the entity's `unknown | null` contract and the test's `expect(auditLog.beforeState).toEqual(input.beforeState)` expectation (which only passes because the fake repository bypasses the real mapRow).
