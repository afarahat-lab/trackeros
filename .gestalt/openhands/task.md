# Fix specific quality-gate violations: Phase 5: Balance module

You are an autonomous coding agent working INSIDE an already-cloned git repository at `/tmp/gestalt/fix/63ff1071-5533-4487-9cf5-cd66e5b8b64e/7/1`. Do not clone anything; work only in this directory.
...
- The new `!Number.isInteger(data.totalEntitlement)` check must follow the same guard pattern as the existing `!Number.isInteger(data.fiscalYear)` check on line 42 of `balance.service.ts` — same predicate, same error-throwing style, same placement within the validation block. (see `src/modules/balance/balance.service.ts`)
### Entity invariants — enforce these
- Reuse or extend `LeaveBalance`: `totalEntitlement` must be a positive integer at creation time. Fractional values are rejected at the service boundary with a `ValidationError` rather than silently floored.
### Interface contract — expose these operations (their shape is yours)
- BalanceService.create — Throws `ValidationError` for: empty/whitespace-only employeeId, empty/whitespace-only leavePolicyId, non-positive or non-finite totalEntitlement, non-integer totalEntitlement, non-positive or non-integer fiscalYear.
...
Unifying change (do this now): Enforce that totalEntitlement must be a positive finite integer by adding !Number.isInteger check in the validation and update the test to expect a ValidationError for fractional input.
...
Line: 44
Offending code: `if (typeof data.totalEntitlement !== 'number' || data.totalEntitlement <= 0 || !Number.isFinite(data.totalEntitlement)) {`
Rule violated: review/spec-violation
Action (do this now): Edit `src/modules/balance/balance.service.ts` at line 44 in place to fix the `review/spec-violation` violation.
What the quality gate found — apply this: [review/spec-violation] Spec success criterion #5 requires `totalEntitlement` to be validated as a "positive finite integer", but the validation is missing `!Number.isInteger(data.totalEntitlement)`. Fractional values like 15.7 pass validation and are silently floored to 15, contradicting the spec's explicit integer requirement. The `fiscalYear` validation on line 48 correctly includes `!Number.isInteger(data.fiscalYear)` — the same check is missing here.

- Site 2
File: tests/unit/modules/balance/balance.service.test.ts
Line: 122
Offending code: `it('should floor fractional totalEntitlement', async () => {`
Rule violated: review/spec-violation
Action (do this now): Edit `tests/unit/modules/balance/balance.service.test.ts` at line 122 in place to fix the `review/spec-violation` violation.
What the quality gate found — apply this: [review/spec-violation] This test validates behavior that contradicts the spec. The spec requires `totalEntitlement` to be a "positive finite integer" and rejects non-integer input with `ValidationError`. This test expects fractional `totalEntitlement` (15.7) to be silently floored to 15 instead of rejected. The test should assert that a fractional `totalEntitlement` throws `ValidationError`.
