...
- `LeaveService` must call `IBalanceService.reserveDays`, `IBalanceService.deductDays`, and `IBalanceService.releaseReservation` with the exact signatures defined in `src/modules/balance/balance.service.interface.ts`. The reservation model (remainingDays decremented on reserve, usedDays incremented on deduct) is owned by BalanceService — LeaveService must not duplicate or bypass it. (see `src/modules/balance/balance.service.interface.ts`)
- `LeaveService` must call `IAuditService.log` with the parameter shape defined in `src/modules/audit/audit.service.interface.ts`: `{ entityType, entityId, action, oldValues, newValues, performedBy }`. The `entityType` must be `'LeaveRequest'`, `action` must reflect the operation name (e.g. `'submitDraft'`, `'approve'`, `'reject'`, `'cancel'`), `oldValues`/`newValues` must capture at minimum the status before and after. (see `src/modules/audit/audit.service.interface.ts`)
- `LeaveService` must use `IEmployeeService.isActive` and `IEmployeeService.getManagerId` as defined in `src/modules/employee/employee.service.interface.ts`. The `isActive` check requires both `employmentStatus === 'ACTIVE'` AND `terminationDate === null` — LeaveService must not reimplement this logic. (see `src/modules/employee/employee.service.interface.ts`)
- `LeaveService` must use the `LeaveRequest` interface exactly as defined in `src/modules/leave/leave.model.ts`. All state transitions must produce `LeaveRequest` objects conforming to this shape, with `status` values from `LeaveRequestStatus` and dates as `Date` objects. (see `src/modules/leave/leave.model.ts`)
- `LeaveService` must use `ILeaveRepository.findOverlapping` with its exact signature `(employeeId, startDate, endDate, excludeId?)`. The overlap query in the repository already filters for SUBMITTED/APPROVED statuses — LeaveService must not add its own status filtering on top. (see `src/modules/leave/leave.repository.ts`)
### Entity invariants — enforce these
- Reuse or extend `LeaveRequest`: Lifecycle: DRAFT → SUBMITTED → (APPROVED | REJECTED). Any non-terminal state (DRAFT, SUBMITTED, APPROVED) → CANCELLED. REJECTED and CANCELLED are terminal — no further transitions allowed. A request in DRAFT status cannot be approved or rejected; only SUBMITTED requests can be approved or rejected.
- Reuse or extend `LeaveRequest`: No two LeaveRequests for the same employee may overlap in date ranges when both have status SUBMITTED or APPROVED. Overlap is defined as `existing.startDate <= new.endDate AND existing.endDate >= new.startDate`.
- Reuse or extend `LeaveRequest`: `approvedBy` and `approvedAt` MUST be set (non-null) when status transitions to APPROVED via explicit manager/HR approval. When auto-approved (`requiresManagerApproval=false`), both MUST remain null.
### Interface contract — expose these operations (their shape is yours)
- submitDraft(requestId, employeeId) — Caller identity (`employeeId`) must match the request's `employeeId`. Only the owning employee may submit their own draft.; Throws `EmployeeNotActiveError` if employee is not ACTIVE. Throws `OverlappingRequestError` if an overlapping SUBMITTED/APPROVED request exists. Throws `MinimumNoticeError` if notice period is insufficient (unless emergency leave). Throws `InsufficientBalanceError` if available balance < requested days. Throws a generic error if the request is not in DRAFT status.
- approve(requestId, approverId, approverRole) — `approverId` must match the employee's `managerId` OR `approverRole` must be `'hr_admin'`. `approverId` must not equal the request's `employeeId` (no self-approval).; Throws `NotManagerError` if the approver is unauthorized. Throws a generic error if the request is not in SUBMITTED status.
- reject(requestId, approverId, approverRole) — Same authorization as `approve`: `approverId` must match the employee's `managerId` OR `approverRole` must be `'hr_admin'`. Self-rejection is forbidden.; Throws `NotManagerError` if the approver is unauthorized. Throws a generic error if the request is not in SUBMITTED status.
- cancel(requestId, employeeId) — `employeeId` must match the request's `employeeId`. Only the owning employee may cancel their own request.; Throws a generic error if the request is already in a terminal state (REJECTED or CANCELLED).
- getById(requestId), getByEmployee(employeeId) — No authorization enforced at the service layer — these are pure read operations. Authorization (if any) is enforced at the controller/route layer.; idempotent; Returns `null` or empty array when no results found; never throws for missing data.
### Integration points — connect to these
- src/modules/employee/employee.service.interface.ts — IEmployeeService.isActive, IEmployeeService.getManagerId — Employee active gate on submit; manager lookup for approval authorization
- src/modules/policy/policy.service.interface.ts — IPolicyService.getById — Look up the LeavePolicy to check minimumNoticeDays, requiresManagerApproval, and leaveType (for emergency bypass)
- src/modules/balance/balance.service.interface.ts — IBalanceService.getAvailableDays, reserveDays, deductDays, releaseReservation — Balance check on submit; reservation on submit; deduction on approve; release on reject/cancel
- src/modules/audit/audit.service.interface.ts — IAuditService.log — Audit every state transition per GP-002
