...
src/modules/notification/notification.{model,repository}.ts   — entity + repository only (no service/index yet)
...
`NotificationRepository` contract from the reconciled architecture above. It
ships only the entity and the repository — no service, no service interface,
no `errors.ts`, and no `index.ts` public entry point.
...
`NotificationService`; the implementation ships only the model and repository,
with no service layer, no `index.ts`, and no `errors.ts`. This diverges from
the established audit/employee/policy/balance pattern (which each add a
service, a service interface, an `errors.ts` and an `index.ts`). The
`NotificationService` remains unbuilt; the module currently has no public
entry point, so other modules cannot yet import from it via `index.ts`.
