...
- **employee** (`src/modules/employee/`) — Employee entity and repository. Service interface deferred.
- **leave-policy** (`src/modules/leave-policy/`) — LeavePolicy entity and repository. Service interface deferred.
- **leave-balance** (`src/modules/leave-balance/`) — LeaveBalance entity, repository, and BalanceService (full implementation). **Note:** barrel export (`index.ts`) does not yet export the service or its interface.
- **notification** (`src/modules/notification/`) — LeaveNotification entity, repository, and NotificationService (full implementation). **Note:** barrel export (`index.ts`) does not yet export the service or its interface.
- **audit** (`src/modules/audit/`) — AuditLog entity, repository, and IAuditService (stub interface only — no implementation). **Note:** barrel export (`index.ts`) does not yet export the service interface.
...
10. **Supporting services** — BalanceService ✅, NotificationService ✅, IAuditService (stub) ✅. Deferred: LeavePolicyService interface, EmployeeService interface, service unit tests, barrel export updates for new services.
...
- **Audit**: All state-changing operations should produce an audit record (GP-002). **Not yet integrated** — the LeaveRequestService does not call audit or notification modules; these cross-cutting concerns are deferred to a future phase.
