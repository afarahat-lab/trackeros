...
### Phase 6.3 delivered (leave read-surface tests)

This sub-phase is test-only: no production source under `src/` was modified. It adds the Jest tests for the read surface delivered in 6.1/6.2, treating those deliverables as fixed contracts.

**repository tests** — `tests/unit/modules/leave/leave.repository.test.ts` (new) exercises the real `PgLeaveRequestRepository.findByQuery` against a fake `dbPool`/`PoolClient` whose `query(sql, values)` records the SQL text and parameter array (the spec's ambiguity resolved in favor of the recording-client option, not a re-implementing fake). Coverage: `employeeIds` is passed as a single `employee_id = ANY($n)` parameter (array pushed as one value, not spread); the `ANY` clause is omitted when `employeeIds` is empty or absent; snake_case rows map to camelCase `LeaveRequest` (with `Date` instances); an empty result set returns `[]` without throwing; and the `ANY($n)` condition is placed before `LIMIT`/`OFFSET` with scalars kept as separate parameters.

**route tests** — `tests/unit/modules/leave/leave.routes.test.ts` (extended) adds `GET /leaves` and `GET /leaves/:id` describe blocks using the existing seam: a Fastify instance decorated with a fake `leaveService` and a `preHandler` that sets `request.user`. Coverage: 200 with the service's array (Date fields compared in serialized form), query-param parsing/forwarding (`status`/`leaveTypeCode` enum membership, `startDateFrom` via `toDate`, `limit`/`offset` as integers), 400 `VALIDATION_ERROR` for invalid `status`/`limit`/`startDateFrom`, 403 `FORBIDDEN` mapping, 404 `NOT_FOUND` for `getById` (identical body for invisible/missing), 500 `Internal Server Error` for a non-AppError, and 401 `UNAUTHORIZED` when `request.user` is missing.

**service tests** — `tests/unit/modules/leave/leave.service.test.ts` (extended) adds a `list`/`getById` describe block reusing the existing in-memory fakes; `FakeLeaveRepository.findByQuery` was extended to record the received params (`findByQueryCalls`) rather than ignore them. Coverage: EMPLOYEE scopes to `[actor.id]`; MANAGER scopes to `[actor.id, ...directReportIds]` (one level, not transitive — second-level reports and other managers' reports excluded); ADMIN passes no `employeeIds` filter; `list` delegates once and returns the repository rows unchanged; `getById` returns a visible request, throws `NotFoundError('Leave request not found')` for a missing request AND for an out-of-scope request (identical message), and makes a direct report's request visible regardless of status (APPROVED/REJECTED/CANCELLED); missing actor → UnauthorizedError, invalid role → ForbiddenError.
