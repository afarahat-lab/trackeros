...
3. **balance and notification** — ✅ DONE (part 1/2). Models + interfaces + repositories for both modules (see implementation notes below). Services, routes, and unit tests are deferred to part 2/2.
...
- **Deferred to part 2/2**: `BalanceService`/`NotificationService` implementations, route files, and Jest specs. The balance→audit dependency (GP-002) is not yet realized in this phase — it lands when the service layer writes audit records.
