...
### Phase 3b delivered (balance read endpoint + policy effective-listing)

This sub-phase delivers the `GET /balances/me` read path: the policy service's effective-policy listing, the balance service's per-employee list method, and the balance routes. It is the second of the split Phase 3 sub-phases; `app.ts` registration and the smoke/route unit tests (3c/Phase 5) are not yet implemented.

**policy** — `IPolicyService` gained `listEffectivePolicies(asOf: Date): Promise<LeavePolicy[]>`; `PolicyService.listEffectivePolicies` calls the **existing** `IPolicyRepository.findAll()` (no new repository method, no SQL in the service, no leave-type module dependency) and filters in application code: a policy is in effect when `status === ACTIVE`, `effectiveFrom <= asOf`, and (`effectiveTo === null` OR `effectiveTo >= asOf`); when multiple rows still match a `leaveTypeCode`, the latest `effectiveFrom` wins — so the result carries **at most one policy per leave type**. This is what prevents a superseded policy from producing duplicate balance entries for the same type.

**balance** — `BalanceEntry` interface (`leaveTypeCode`, `periodStart`, `periodEnd`, `entitledDays`, `usedDays`, `pendingDays`, `available`). `IBalanceService` gained `getBalanceForEmployee(employeeId: string): Promise<BalanceEntry[]>`. The implementation: `getEmployeeById(employeeId)`, then `listEffectivePolicies(new Date())`, and for each effective policy computes the current period via the shared `periodContaining(employee.hireDate, policy.accrualPeriodMonths, new Date())` and looks the balance up with `findByKey`. A leave type with an effective policy but **no balance row for the current period is returned with zeroes** (`entitledDays`/`usedDays`/`pendingDays` = 0, `available` = 0) — it is neither omitted nor a NotFoundError. `available = entitledDays - usedDays - pendingDays`. Read-only: no transaction, no writes.

**routes** — `balance.routes.ts` (NEW): `balanceRoutes(fastify)` registers `GET /balances/me` (200). `resolveActor` extracts `request.user` (never a client-supplied id) and enforces role membership at the API boundary (GP-005); `sendError` maps `AppError` → `{ error, code }` and any other throw to 500. `createBalanceService()` wires `PgLeaveBalanceRepository`, `EmployeeService(PgEmployeeRepository)`, `PolicyService(PgLeavePolicyRepository, LeaveTypeService(PgLeaveTypeRepository))`, and `PgUnitOfWork`. The service is resolved from `fastify.balanceService` if present, else `createBalanceService()`.

**index.ts** — re-exports `balanceRoutes` and `createBalanceService` (routes) and `BalanceEntry` (service).

**Divergences from the plan worth noting:**
- `app.ts` was **not** modified — `balanceRoutes` is exported but not registered, so `GET /balances/me` is not mounted at runtime (the same unreachable-endpoint issue the `app.ts` comment describes for `leaveRoutes`, which was fixed for leave but not yet for balance).
- The verification brief required the smoke check to seed at least two leave types with effective policies and balances; `scripts/smoke.js` was not touched, so no smoke assertions exist for `GET /balances/me`.
- No unit tests were added for `getBalanceForEmployee` — the two touched test files (`balance.service.test.ts`, `leave.service.test.ts`) changed only to add `listEffectivePolicies` to their `FakePolicyService` (satisfying the widened `IPolicyService` under `tsc --noEmit`). No new assertions.
- The effective-policy filtering is done in the service over `findAll()` (application code) rather than as a filtered SQL query in the repository — permitted by the brief (no new repository method, no SQL inside a service), but it loads every policy row and filters in memory.
- `BalanceService` now imports both `periodContaining` and `addMonths` from `src/shared/date/accrual.ts` — the private `addMonths` duplicate that 3a deferred was consumed in this sub-phase, so the shared module is now the sole copy of that arithmetic (the 3a divergence note above is superseded).
- The "omit vs zeroes" decision for a missing balance row was resolved in favour of **zeroes** (return the entry with zeroed counters), applied consistently across all effective policies.
