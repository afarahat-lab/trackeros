...
4. **leave orchestration module** — ✅ DONE. Composes the leave-request repository, the balance repository, and the unit of work inside `LeaveService`. NOTE (divergence): the committed implementation does NOT realize the full orchestration described in the reconciled architecture — it performs lifecycle transitions + sufficiency + overlap checks only, and does NOT deduct balance, write audit records, or create notifications (see implementation notes below).
...
### Implementation notes (leave orchestration module)
- **Files**: `src/modules/leave/leave.model.ts`, `leave.repository.ts`, `leave.service.ts`, `leave.routes.ts`, `index.ts`, plus `tests/unit/modules/leave/leave.service.spec.ts`.
- **Model** (`leave.model.ts`): `LeaveRequest` entity with `id`, `employeeId`, `leaveTypeId`, `startDate`, `endDate`, `reason` (`string | null`), `status` (`LeaveRequestStatus`), `approvedBy` (`string | null`), `approvedAt` (`Date | null`), `createdAt`, `updatedAt`. Three typed errors: `InvalidLeaveRequestTransitionError`, `InsufficientLeaveBalanceError`, `OverlappingLeaveError`. `ILeaveRequestRepository` (`create`, `findById`, `findByEmployee`, `update`, `list`) and `ILeaveService` (`apply`, `approve`, `reject`, `cancel`, `list`). `CreateLeaveRequestInput` = `Omit<LeaveRequest, 'id' | 'status' | 'approvedBy' | 'approvedAt' | 'createdAt' | 'updatedAt'>`.
- **Repository** (`leave.repository.ts`): `PgLeaveRequestRepository` with snake_case columns (`employee_id`, `leave_type_id`, `start_date`, `end_date`, `approved_by`, `approved_at`, `created_at`, `updated_at`). `update` builds a dynamic `SET` clause from a `COLUMNS` map (excluding `id`), always stamps `updated_at = NOW()`, and returns the updated row (or `null`). All methods take an optional `PoolClient` last param and fall back to the shared `pool`.
- **Service** (`leave.service.ts`): `LeaveService` constructor takes `PgLeaveRequestRepository` (concrete) + `ILeaveBalanceRepository` + `IUnitOfWork`. `apply` generates `id`/`createdAt`/`updatedAt`, sets `status = PENDING`, `approvedBy`/`approvedAt = null`, and delegates to `repo.create` WITHOUT a transaction. `approve`/`reject`/`cancel` use the `client ? run(client) : this.uow.withTransaction(run)` pattern (join a caller's transaction when a client is supplied, otherwise open their own).
- **Lifecycle (as built)**: `PENDING → APPROVED | REJECTED`; `PENDING | APPROVED → CANCELLED`. Any other transition throws `InvalidLeaveRequestTransitionError`. `approve`/`reject`/`cancel` return `null` when the request does not exist.
- **Approve flow (as built)**: (1) `findById` → `null` returns `null`; (2) status must be `PENDING` else `InvalidLeaveRequestTransitionError`; (3) `days = countLeaveDays(startDate, endDate)`; (4) `findByEmployee` for the employee's requests; (5) `balances.findByEmployee` and select the balance where `policyId === leaveTypeId` — if none, `InsufficientLeaveBalanceError`; (6) `pendingDays` = sum of `countLeaveDays` over OTHER `PENDING` requests of the SAME `leaveTypeId`; (7) `availableDays = computeAvailableDays(totalEntitlement, usedDays, pendingDays)`; (8) if `days > availableDays` → `InsufficientLeaveBalanceError`; (9) overlap check: any OTHER `APPROVED` request whose range overlaps (`aStart <= bEnd && bStart <= aEnd`) → `OverlappingLeaveError`; (10) `update` to `APPROVED` with `approvedBy`/`approvedAt`.
- **Routes** (`leave.routes.ts`): instantiates `LeaveService(new PgLeaveRequestRepository(), new PgLeaveBalanceRepository(), new UnitOfWork())`; Zod `safeParse` validation (400 `VALIDATION_ERROR`). Endpoints and RBAC:
  - `POST /leaves` (EMPLOYEE, MANAGER, HR_ADMIN) → 201; `applyLeaveSchema` refines `endDate >= startDate`.
  - `POST /leaves/:id/approve` (MANAGER, HR_ADMIN) → 200; `null` → 404 `NOT_FOUND`.
  - `POST /leaves/:id/reject` (MANAGER, HR_ADMIN) → 200; `null` → 404.
  - `POST /leaves/:id/cancel` (EMPLOYEE, MANAGER, HR_ADMIN) → 200; `null` → 404.
  - `GET /leaves` (MANAGER, HR_ADMIN) → 200.
  - NOTE (divergence from the standard error contract): `InvalidLeaveRequestTransitionError`, `InsufficientLeaveBalanceError`, and `OverlappingLeaveError` all map to HTTP 400 with code `CONFLICT` (not `VALIDATION_ERROR`).
- **Divergences from the reconciled architecture (documented as built)**:
  - **No balance deduction on approve**: `approve` does NOT call `balance.deduct` — the "approving deducts `usedDays`/`remainingDays` atomically" rule is NOT implemented. The sufficiency check reads the balance but never mutates it.
  - **No audit record (GP-002 not realized)**: `LeaveService` has no `IAuditService` dependency and writes no `AuditLog` on any state change.
  - **No notification**: `LeaveService` has no `INotificationService` dependency and creates no notification on approve/reject.
  - **No manager-relationship check**: `approvedBy` is accepted as a body field and is NOT verified against `Employee.managerId`; RBAC only requires the caller's role to be MANAGER or HR_ADMIN.
  - **No `employmentStatus === ACTIVE` check** on `apply`, and **no `LeavePolicy.isActive` check** on the referenced leave type.
  - **Cancel does not restore days**: since nothing is deducted on approve, `cancel` of an `APPROVED` request performs no balance restore.
  - **Balance resolution**: the applicable balance is matched by `policyId === leaveTypeId` (a direct string match), NOT resolved via `leave_type` + `is_active` as the canonical decision states.
  - **Dependency surface**: `LeaveService` depends only on `PgLeaveRequestRepository` + `ILeaveBalanceRepository` + `IUnitOfWork` — it does NOT depend on notification, audit, employee, or policy modules as the reconciled "Modules and dependencies" section declares.
- **Tests** (`tests/unit/modules/leave/leave.service.spec.ts`): covers `apply` (assigns `id`/`createdAt`/`updatedAt`, `status=PENDING`, null `approvedBy`/`approvedAt`, delegates to `repo.create`); lifecycle transitions (approve/reject/cancel happy paths + illegal transitions throwing `InvalidLeaveRequestTransitionError` + `null` on missing request); sufficiency check (approve when `days <= availableDays`, reject when exceeding, reject when `availableDays` is negative — no clamping, and counting other `PENDING` same-type requests against availability); overlap check (approve when no overlap, reject on overlap regardless of type, ignore `PENDING`/`REJECTED`); and `list` delegation. Uses `jest.Mocked<ILeaveRequestRepository>` + `jest.Mocked<ILeaveBalanceRepository>` + `jest.Mocked<IUnitOfWork>` with `withTransaction.mockImplementation(async (fn) => fn(fakeClient))`.
