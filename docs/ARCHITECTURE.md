...
src/modules/leave-balance/leave-balance.{model,repository,service.interface,service}.ts
...
- **LeaveBalance** (`src/modules/leave-balance/`): model, repository (PgLeaveBalanceRepository), service interface (ILeaveBalanceService: getBalance, initializeBalancesForEmployee, deductOnApproval, releaseOnRejectionOrCancellation, getRemainingDays), service (LeaveBalanceService — injects ILeaveBalanceRepository + ILeavePolicyRepository; all methods accept optional PoolClient for caller-controlled transactions).
...
- **Phase 8: LeaveBalanceService** — ✅ Complete. `src/modules/leave-balance/leave-balance.service.interface.ts` defines `ILeaveBalanceService` (getBalance, initializeBalancesForEmployee, deductOnApproval, releaseOnRejectionOrCancellation, getRemainingDays). All methods accept an optional `PoolClient` for caller-controlled transactions. `src/modules/leave-balance/leave-balance.service.ts` delivers `LeaveBalanceService` — injects `ILeaveBalanceRepository` + `ILeavePolicyRepository`. `getBalance` resolves the active policy for the leave type, then looks up the balance by employee+policy+fiscalYear; returns `null` when no active policy or no balance record exists. `initializeBalancesForEmployee` iterates all active policies, calculates pro-rated entitlement per policy, and creates a balance record for each. Fiscal year is derived from `employee.hireDate.getUTCFullYear()`. Pro-rating formula: `Math.floor(entitlementDays * wholeMonthsRemaining / 12)` where `wholeMonthsRemaining = 11 - hireMonth` (0-indexed); hires in December get 0 entitlement with EXHAUSTED status; hires in prior fiscal years get full entitlement. `deductOnApproval` validates the active policy exists, the balance record exists, and `remainingDays >= days`, then delegates to `balanceRepository.deductDays`. `releaseOnRejectionOrCancellation` validates the active policy and balance record exist, then delegates to `balanceRepository.restoreDays`. `getRemainingDays` delegates to `getBalance` and returns `0` when no balance exists. All error paths throw descriptive `Error` instances (not typed domain errors). Tests in `tests/unit/modules/leave-balance/leave-balance.service.test.ts` (22 tests covering all methods, edge cases, PoolClient passthrough, and error paths).
