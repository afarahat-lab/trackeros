...
3. **balance and notification** — balance ✅ DONE (parts 1/2 + 2/2); notification ⏳ part 1/2 DONE, part 2/2 PENDING. Balance models + interfaces + repository + service + routes are complete (see implementation notes below). Notification models + interfaces + repository are complete; `NotificationService`, `notification.routes.ts`, and Jest specs remain deferred.
...
- **Deferred (notification part 2/2)**: `NotificationService`, `notification.routes.ts`, and Jest specs for both balance and notification services are NOT built in this phase. `notification/index.ts` still exports only model + repository symbols.
