...
const COLUMNS = `id, employee_id, leave_type_id, start_date, end_date, reason, status,
...
function mapRow(row: LeaveRequestRow): LeaveRequest {
  return {
    ...
    status: isLeaveRequestStatus(row.status)
      ? row.status
    ...
    const result = await db.query(
      `SELECT ${COLUMNS} FROM leave_requests
       WHERE employee_id = $1
         ...
         AND start_date <= $3
         AND end_date >= $4`,
      [employeeId, LeaveRequestStatus.APPROVED, endDate, startDate]
    );
