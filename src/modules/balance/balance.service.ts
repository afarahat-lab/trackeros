...
export class BalanceService implements IBalanceService {
  ...
  async reserveDays(employeeId: string, policyId: string, year: number, days: number): Promise<void> {
    ...
    await this.repository.updateCounters(balance.id, balance.usedDays, balance.pendingDays + days);
  }

  async commitDays(employeeId: string, policyId: string, year: number, days: number): Promise<void> {
    const balance = await this.getBalanceForOperation(employeeId, policyId, year);
    if (balance.pendingDays < days) {
      throw new Error(`Insufficient pending days: requested ${days}, pending ${balance.pendingDays}`);
    }
    await this.repository.updateCounters(balance.id, balance.usedDays + days, balance.pendingDays - days);
  }

  async releaseDays(employeeId: string, policyId: string, year: number, days: number): Promise<void> {
    const balance = await this.getBalanceForOperation(employeeId, policyId, year);
    if (balance.pendingDays < days) {
      throw new Error(`Insufficient pending days: requested ${days}, pending ${balance.pendingDays}`);
    }
  ...
  async restoreDays(employeeId: string, policyId: string, year: number, days: number): Promise<void> {
    const balance = await this.getBalanceForOperation(employeeId, policyId, year);
    if (balance.usedDays < days) {
      throw new Error(`Insufficient used days: requested ${days}, used ${balance.usedDays}`);
    }
    await this.repository.updateCounters(balance.id, balance.usedDays - days, balance.pendingDays);
  }
